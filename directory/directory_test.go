...
func TestGetPutBlob(t *testing.T) {
	...
	info, err := dest.PutBlob(context.Background(), bytes.NewReader(blob), types.BlobInfo{Digest: digest.Digest("sha256:digest-test"), Size: int64(9)}, 0, cache, false, nil)
	assert.NoError(t, err)
	err = dest.Commit(context.Background())
	assert.NoError(t, err)
...
func TestPutBlobDigestFailure(t *testing.T) {
	...
	require.NoError(t, err)
	defer dest.Close()
	_, err = dest.PutBlob(context.Background(), reader, types.BlobInfo{Digest: blobDigest, Size: -1}, 0, cache, false, nil)
	assert.Error(t, err)
