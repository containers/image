...
func SearchRegistry(ctx context.Context, sys *types.SystemContext, registry, image string, limit int) ([]SearchResult, error) {
	...
	if image != "" {
		...
		resp, err := client.makeRequest(ctx, "GET", u.String(), nil, nil, v1Search)
		if err != nil {
...
func (c *dockerClient) detectProperties(ctx context.Context) error {
	...
	ping := func(scheme string) error {
		...
		resp, err := c.makeRequestToResolvedURL(ctx, "GET", url, nil, nil, -1, v2Auth)
		if err != nil {
	...
	if err != nil {
		...
		pingV1 := func(scheme string) bool {
			...
			resp, err := c.makeRequestToResolvedURL(ctx, "GET", url, nil, nil, -1, v1Basic)
			logrus.Debugf("Ping %s err %#v", url, err)
			if err != nil {
				return false
			}
