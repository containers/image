...
func TestPKIVerify(t *testing.T) {
	...
	// Failure intermediates with neither signature nor config
	...
	// Success intermediate certificate with policy config only
	pk, err = verifyPKI(&pkiTrustRoot{
		caRootsCertificates:        caRootsCertificates,
		caIntermediateCertificates: caIntermediateCertificates,
	}, certBytes, []byte{})
	...
	// Success intermediate certificate with signature only
	pk, err = verifyPKI(&pkiTrustRoot{
		caRootsCertificates: caRootsCertificates,
