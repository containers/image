...
func Test_rekorUploadKeyOrCert(t *testing.T) {
	// setup an external rekor server running with podman
	nameSuffix := stringid.GenerateNonCryptoID()[:12]
	cmd := exec.Command("./scripts/start-rekor.sh", nameSuffix)
	cmd.Stderr = os.Stderr
	cmd.Stdout = os.Stdout
	err := cmd.Run()
	require.NoError(t, err, "setup rekor server with podman")

	t.Cleanup(func() {
		cmd := exec.Command("podman", "pod", "rm", "-f", "-t0", "rekor-pod-"+nameSuffix)
		cmd.Stderr = os.Stderr
		cmd.Stdout = os.Stdout
		err := cmd.Run()
		require.NoError(t, err, "stop rekor server with podman")
	})

	REKOR_SERVER := "http://127.0.0.1:3000"
